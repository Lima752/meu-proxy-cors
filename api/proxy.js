...
// VERSÃO FINAL CORRIGIDA - clientId corrigido (m -> r)
...
module.exports = async (request, response) => {
    ...
    console.log("--- INICIANDO REQUISIÇÃO (clientId CORRIGIDO) ---");

    try {
        const { id } = request.query;
        if (!id) { return response.status(400).json({ success: false, msg: "Device ID é obrigatório." }); }

        // ===== clientId CORRIGIDO AQUI =====
        const clientId = "8y9cawst5kr45canysuq"; // Corrigido m para r
        // ===================================
