...
// VERSÃO FINAL CORRIGIDA - Client Secret CORRIGIDO
...
module.exports = async (request, response) => {
    ...
    console.log("--- INICIANDO REQUISIÇÃO (Client Secret CORRIGIDO) ---");

    try {
        const { id } = request.query;
        if (!id) { return response.status(400).json({ success: false, msg: "Device ID é obrigatório." }); }

        const clientId = "8y9cawst5kr45canysuq"; // CORRETO
        // ===== Client Secret CORRIGIDO AQUI =====
        const secret = "2cabe1ab28674f1f9b10376baf22e94f"; // CORRIGIDO 8 para 3
        // ======================================
        const baseUrl = "https://openapi.tuyaus.com";
        const t = Date.now().toString();
        
        // ===== PASSO 1: OBTER O ACCESS TOKEN (Assinatura correta) =====
